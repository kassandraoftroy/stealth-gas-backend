...
impl DbClient {
    ...
    pub async fn get_msg_ids_from_spend_id(&self, spend_id: i32) -> Result<Vec<String>, String> {
        sqlx::query("SELECT message_id FROM tickets WHERE spend_id = $1")
            .bind(spend_id)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn update_spend_state(&self, row_id: i32, state: EventState) -> Result<(), String> {
        sqlx::query(
    ...
    pub async fn update_spend_tx_data(
        &self,
        spend_id: i32,
        tx_hash: &str,
        tx_nonce: i64,
